...
const sendUsersData = require("./helpers/sendUsersData");
...
bot.start(async (ctx) => {
  queue.enqueue(async () => {
    ...
    let linkFirstChunk = "t.me/tonfantoken02_bot?start=";
  ...
  queue.enqueue(async () => {
    ...
    if (ctx.message.text.trim()) {
      ...
      if (!isWalletValid) {
        ...
        const newReferralLink = `t.me/tonfantoken02_bot?start=${uuidv1()}`;
